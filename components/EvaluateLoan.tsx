...
import { saveEvaluation } from '../services/evaluationDatabase';

import { AlertCircle, Zap, TrendingUp, Info, Leaf, CheckCircle2 } from 'lucide-react';
...
const EvaluateLoan: React.FC<{ onComplete: (res: LoanEvaluationResult) => void }> = ({ onComplete }) => {
  ...
  const handleRunAI = async () => {
    ...
    try {
      // Direct call without timeout first to catch immediate errors, 
      // or use a wrapper to handle the async error inside the timeout.
      // Better: await a promise that resolves after min 3s OR fails.

      const startTime = Date.now();
      const result = await evaluateLoan(financial, sustainability);
      const elapsed = Date.now() - startTime;

      await saveEvaluation(financial, sustainability, result).catch(() => { });
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 pb-12">
      {error && (
        <div className="p-4 bg-red-500/10 border border-red-500/50 rounded-xl flex items-center gap-3 text-red-500 animate-in fade-in slide-in-from-top-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <GlassCard delay={0.1}>
          ...
          <InputField
            ...
            onChange={(v: number) => setFinancial(p => ({ ...p, loanAmount: v }))}
            suffix="$"
          />
        ...
        <GlassCard delay={0.2}>
          ...
          <div className="mb-6">
            ...
            <input
              ...
              min="0"
              max="100"
              value={sustainability.wasteScore}
              onChange={(e) => setSustainability(p => ({ ...p, wasteScore: Number(e.target.value) }))}
              className="w-full accent-electricGreen h-1.5 bg-white/10 rounded-lg appearance-none cursor-pointer"
            />
            <p className="text-slate-400 text-sm mt-1 mono">{sustainability.wasteScore}</p>
          </div>
